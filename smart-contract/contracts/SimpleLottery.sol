...
contract SimpleLottery is Ownable {
    struct Lottery {
        ...
        uint256 winningAmount; // winning amount is in wei
    ...
    ) public returns (uint256 lotteryId) {
        /*
         * 1.  Check that the caller of the function is the contract owner by using the onlyOwner modifier, also use canStart modifier to check if previous lotteries are ended.
    ...
    function buy() public returns (string memory ticketId) {
        /*
         * 1.  Use "buyAllowed" modifier to validate the lottery status.
    ...
    function declareWinner() public returns (string memory winnerTicketId) {
        /**
         * 1.  Check that the caller of the function is the contract owner by using the onlyOwner modifier, also use isLotteryFull modifier to check if lottery is full.
